...
# types
fadd_t        = Fxp(None, signed=True, dtype=f'fxp-s{BIT_WIDTH+1}/0',                     overflow='wrap')
fcplx_add_t   = Fxp(None, signed=True, dtype=f'fxp-s{BIT_WIDTH+1}/0-complex',             overflow='wrap')
fmult_t       = Fxp(None, signed=True, dtype=f'fxp-s{2*BIT_WIDTH}/{BIT_WIDTH-1}',         overflow='wrap')
fcmplx_mult_t = Fxp(None, signed=True, dtype=f'fxp-s{2*BIT_WIDTH}/{BIT_WIDTH-1}-complex', overflow='wrap')
fint_t        = Fxp(None, signed=True, dtype=f'fxp-s{BIT_WIDTH}/0',                       overflow='wrap')
fcmplx_int_t  = Fxp(None, signed=True, dtype=f'fxp-s{BIT_WIDTH}/0-complex',               overflow='wrap')
ftf_t         = Fxp(None, signed=True, dtype=f'fxp-s{BIT_WIDTH}/{BIT_WIDTH-1}',           overflow='wrap')
fcmplx_tf_t   = Fxp(None, signed=True, dtype=f'fxp-s{BIT_WIDTH}/{BIT_WIDTH-1}-complex',   overflow='wrap')

def unpack_scomplex8_t(v : BinaryValue):
    v_as_str = v.binstr
    return v_as_str[BIT_WIDTH:],f"{v_as_str[0:BIT_WIDTH]}j"
...
def fixed_point_multiply(a, b):
    ...
    out.equal(a * b)
    return Fxp(f"0b{out.bin()[1:BIT_WIDTH+1]}").like(fint_t)
